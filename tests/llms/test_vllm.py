...
def test_generate_response_without_tools(mock_vllm_client):
    config = BaseLlmConfig(model="google/gemma-3-1b-it", temperature=0.7, max_tokens=100, top_p=1.0)
    llm = VllmLLM(config)
    messages = [
    ...
    mock_vllm_client.chat.completions.create.assert_called_once_with(
        model="google/gemma-3-1b-it", messages=messages, temperature=0.7, max_tokens=100, top_p=1.0
    )
